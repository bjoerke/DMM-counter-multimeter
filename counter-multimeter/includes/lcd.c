...
void LCD_Init(void) {
	...
	/* Set Register Select and Chip Select as Output */DDRC |= (1 << PC7)
			| (1 << PC6);
	...
	/* Starting Init Command Sequence */LCD_Command_Mode;
	LCD_Chip_Select;
	...
	/* Restore Status Register */SREG = sreg;
...
void LCD_PutChar(const char c) {
	...
	default:
		for (uint8_t x = 0; x < 6; x++)
			lcd_framebuffer[lcd_texty][(lcd_textx * 6) + x] =
					pgm_read_byte(&font[(uint8_t)(c)][x]);
...
void LCD_PutCharLarge(const char c) {
	// basic support for cr und new line
	switch (c) {
	case '\r':
		// Carriage Return
	...
	default:
		for (uint8_t x = 0; x < 12; x++) {
			lcd_framebuffer[lcd_texty][(lcd_textx * 6) + x] =
					pgm_read_byte(&fontLarge[(uint8_t)(c)][x * 2 + 1]);
			lcd_framebuffer[lcd_texty + 1][(lcd_textx * 6) + x] =
					pgm_read_byte(&fontLarge[(uint8_t)(c)][x * 2]);
		}
		lcd_frameupdate |= (0x03 << lcd_texty);
...
void LCD_PutStringLarge(const char *s) {
	// no empty strings allowed!
	if (*s) {
		do {
			LCD_PutCharLarge(*s);
...
void LCD_PutString_PLarge(PGM_P s)
{
	while(1)
	{
		unsigned char c = pgm_read_byte (s);
