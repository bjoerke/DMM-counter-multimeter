...
int main(void) {
	...
	while (1) {
		PORTB &= ~(1 << PB3);
		uint32_t freq = cnt_TakeMeasurement(COUNTER_RANGE_AUTO);
		uint32_t duty = counter_MeasureDuty(1000);
		PORTB |= (1 << PB3);
		LCD_Clear();
		LCD_GotoXY(0, 0);
		LCD_PutString_P(PSTR("CounterFreq+Duty test"));
		LCD_GotoXY(0, 2);
		LCD_PutString_P(PSTR("Hz:"));
		LCD_PutInteger(freq);
		LCD_GotoXY(0, 3);
		LCD_PutString_P(PSTR("Duty:"));
		LCD_PutInteger(duty/10);
		LCD_PutChar('.');
		LCD_PutChar(duty%10+'0');
		LCD_PutChar('%');
		LCD_Update();
		time_Waitms(500);
	}
